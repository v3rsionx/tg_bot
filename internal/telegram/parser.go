...
type ParsedCommand struct {
	// Name is the command without the leading slash (for example "start").
	Name string
	// Args are whitespace-separated arguments following the command.
	Args []string
	// Raw is the original message text.
	Raw string
	// IsCommand reports whether the text was recognized as a command.
	IsCommand bool
}

// ParseCommand extracts a command and arguments from Telegram message text.
// Mentions such as /start@MyBot are normalized to "start".
